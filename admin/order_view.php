...
if (isset($_GET['delete'])) {
   ...
   $update_order->execute([$order_status, $id]);
...
if (isset($_GET['delete2'])) {
   ...
   $affected_rows = $delete_order->rowCount();
   if ($affected_rows > 0) {
      $success_msg[] = 'order record deleted successfully!';
      header('location:order_view.php');
      exit();
   } else {
      $error_msg[] = 'Failed to delete the record.';
   }
}
...
<div class="box">
   ...
   <div class="box_container">
      ...
      <div class="column">
      <a href="../admin/user_detail.php?user_id=<?= $fetch_admin['id']; ?>">
      <img src="../images/<?= $fetch_admin['image']; ?>" class="image" alt=""></a>
            <div>
            <a href="../admin/user_detail.php?user_id=<?= $fetch_admin['id']; ?>"><?= $fetch_admin['name']; ?></a>
         ...
         <?php } ?>
         <div class="status" style="background-color:<?php if ($fetch_order['status'] == 'waiting for approval from centre') {echo '#D4AF37'; } else if ($fetch_order['status'] == 'waiting for home visit/interview') {echo 'orange'; } else if ($fetch_order['status'] == 'cancelled') {echo 'red'; }else {echo 'limegreen';}; ?>;"><?= $fetch_order['status']; ?></div>
         ...
         <div class="flex-btn">
            <a href="order_edit.php?id=<?= $order_id; ?>" class="option-btn">edit/view</a>
            <?php if ($fetch_order['status'] != 'cancelled') { ?>
    <a href="order_view.php?id=<?= $order_id; ?>&delete=1" class="delete-btn" onclick="return delete_msg(event);">cancel</a>
<?php } else { ?>
    <a href="order_view.php?id=<?= $order_id; ?>&delete2=1" class="delete-btn" onclick="return delete_msg(event);">delete</a>
<?php } ?>
         </div>
      </form>
      <?php
