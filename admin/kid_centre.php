...
<?php
   ...
   <h1 class="heading">Pet of - <?php echo $heading; ?></h1>
...
<?php } ?>
   ...
   <div class="box_container">
      ...
      <?php
      if ($role == 1) {
         $kid = $conn->prepare("SELECT * FROM `kid` WHERE centre_id = ? and status = 'waiting for owner'");
         $kid->execute([$centre_id]);
      } else{
         $kid = $conn->prepare("SELECT * FROM `kid` WHERE centre_id = ? and status = 'waiting for owner' and centre_id = ?");
         $kid->execute([$centre_id, $id]);
      }
         if ($kid->rowCount() > 0) {
