...
<form action="contribution_search.php" method="post" class="search">
      <input type="text" name="search_contribution" maxlength="100" placeholder="search report..." required>
      <button type="submit" name="search_contribution_btn" class="fas fa-search"></button>
   </form>
   <div class="box_container">

      <?php
            if(isset($_POST['search_contribution']) or isset($_POST['search_contribution_btn'])){
               $search_contribution = $_POST['search_contribution'];
               $search_contribution = "%{$search_contribution}%";
               $report = $conn->prepare("SELECT * FROM `contribute` WHERE title LIKE ? OR id LIKE ?and centre_id = ?");
               $report->execute([$search_contribution, $search_contribution, $_SESSION['id']]);
      ...
      <img src="../images/<?= $fetch_admin['image']; ?>" class="image" alt=""></a>
            ...
            <img src="../images/<?= $fetch_report['image']; ?>" class="article_image" alt="" onclick="displayImage(this)">
         ...
         <div class="report"><i>Description: </i><?= $fetch_report['description']; ?></div>
         <div class="flex-btn">
