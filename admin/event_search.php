...
<section class="ar_view">
   ...
   <div class="box_container">
      ...
      <?php
            if(isset($_POST['search_event']) or isset($_POST['search_event_btn'])){
               $search_event = $_POST['search_event'];
               $search_event = "%{$search_event}%";
               $event = $conn->prepare("SELECT * FROM `event` WHERE title LIKE ? and status = 'published'");
               $event->execute([$search_event]);
      ...
      if ($role == 1) {
         ...
         $event = $conn->prepare("SELECT * FROM `event` WHERE status = 'published' AND admin_id = ? ORDER BY date DESC");
         $event->execute([$id]);
      }
         if ($event->rowCount() > 0) {
      ...
      <img src="../images/<?= $fetch_admin['image']; ?>" class="image" alt=""></a>
         ...
         <?php if ($role == 1 or $fetch_event['admin_id'] == $id) { ?>
         <div class="flex-btn">
