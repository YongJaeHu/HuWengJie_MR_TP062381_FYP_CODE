...
<?php
   ...
   <h1 class="heading">Article Status - <?php echo $heading; ?></h1>
...
<?php } ?>
   ...
   <div class="box_container">
      ...
      <?php
      $article = $conn->prepare("SELECT * FROM `article` WHERE status = ? AND admin_id = ?");
      $select_admin = $conn->prepare("SELECT * FROM `admin` WHERE id = ?");
      $select_admin->execute([$id]);
      $fetch_admin = $select_admin->fetch(PDO::FETCH_ASSOC);
      $role = $fetch_admin['role'];
      // if ($role == 1) {
      //    $article->execute([$status]);
      // } else {
         $article = $conn->prepare("SELECT * FROM `article` WHERE status = ? AND admin_id = ? ORDER BY date DESC");
         $article->execute([$status, $id]);
      // }
      ...
      <img src="../images/<?= $fetch_admin['image']; ?>" class="image" alt="">
            <div>
            <a href="../admin/admin_detail.php?id=<?= $fetch_article['id']; ?>"><?= $fetch_admin['name']; ?></a>
      ...
      <?php
            ...
            echo '<p class="empty">no article added yet! <a href="article_add.php" class="btn" style="margin-top:1.5rem;">add article</a></p>';
         }
      ?>
