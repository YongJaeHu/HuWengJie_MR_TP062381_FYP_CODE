...
<?php
   ...
   <div class="box_container">
      ...
      <?php
         $report= $conn->prepare("SELECT * FROM `contact` where status = 'unreply' and sender_id = ?");
         $report->execute([$id]);
         if ($report->rowCount() > 0) {
            while ($fetch_report= $report->fetch(PDO::FETCH_ASSOC)) {
               ...
               $select_user->execute([$fetch_report['receiver_id']]);
               $fetch_user = $select_user->fetch(PDO::FETCH_ASSOC);

               $select_admin = $conn->prepare("SELECT * FROM `admin` WHERE id = ?");
               $select_admin->execute([$fetch_report['receiver_id']]);
               $fetch_admin = $select_admin->fetch(PDO::FETCH_ASSOC);

      ?>
      <form method="post" class="box">
      <input type="hidden" name="report_id" value="<?= $report_id; ?>">
      <div class="title"><i>To: </i></div></a>
      <div class="column">
