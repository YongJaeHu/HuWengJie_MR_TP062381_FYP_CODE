...
if(isset($_POST["submit"])){
   ...
   if($admin->rowCount() > 0){
      ...
      if($password != $cpass){
         ...
         $insert_admin = $conn->prepare("INSERT INTO `admin`(id, name, location, email, password, image, phone, address, role) VALUES(?,?,?,?,?,?,?,?,?)");
         $insert_admin->execute([$admin_id, $name, $location, $email, $password, $re_image , $phone, $address, $role]);
...
<section class="norm_container">
   ...
   <h1 class="heading">Registration Form For Admin</h1>
   ...
   <a href="../admin/admin_page.php" class="exit-btn">Back</a>
