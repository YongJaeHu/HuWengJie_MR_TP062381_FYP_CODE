...
if (isset($_GET['delete'])) {
   ...
   if ($affected_rows > 0) {
      ...
      $error_msg[] = 'Failed to delete the report.';
   }
}
...
<?php
   ...
   <div class="box_container">
      ...
      <?php
      if(isset($_POST['search_report']) or isset($_POST['search_report_btn'])){
         $search_report = $_POST['search_report'];
         $search_report = "%{$search_report}%";
         if ($role == 0) {
         $report = $conn->prepare("SELECT * FROM `report` WHERE id LIKE ? and centre_id = ?");
         $report->execute([$search_report, $_SESSION['id']]);
      } else {
         $report = $conn->prepare("SELECT * FROM `report` WHERE id LIKE ?");
         $report->execute([$search_report]);
      }
