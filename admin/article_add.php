...
$select_post = $conn->prepare("SELECT * FROM `article` WHERE title = ? AND admin_id = ?");
$select_post->execute([$title, $admin_id]);


if($select_post->rowCount() > 0){
   $error_msg[] = 'Title of article is repeated!';
}else{
   $insert_post = $conn->prepare("INSERT INTO `article`(id, admin_id, title, content, image, image2, image3, image4, status) VALUES(?,?,?,?,?,?,?,?,?)");
   $insert_post->execute([$art_id, $admin_id, $title, $content, $re_image, $re_image2, $re_image3, $re_image4, $status]);
   move_uploaded_file($image_tmp_name, $image_folder);
   $success_msg[] = 'Article Published!';
