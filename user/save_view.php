...
<div class="box">
   ...
   <div class="box_container">
      <?php
      $save = $conn->prepare("SELECT * FROM `save` WHERE user_id = ?");
      $save->execute([$id]);
      if($save->rowCount() > 0){
      while($fetch_save = $save->fetch(PDO::FETCH_ASSOC)){
      $kid = $conn->prepare("SELECT * FROM `kid` where id =? AND status = 'waiting for owner' ORDER BY date DESC");
         $kid->execute([$fetch_save['kid_id']]);
         if ($kid->rowCount() > 0) {
      ...
      echo '<p class="empty">no saved pet!</p>';
   }
      ?>
