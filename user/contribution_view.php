...
//    }
//    $delete_contribute= $conn->prepare("DELETE FROM `contact` WHERE id = ?");
...
<h1 class="heading">Contribution list</h1>
   <div class="box_container">
      ...
      <?php
         $contribute = $conn->prepare("SELECT * FROM `contribute` WHERE sender_id = ?");
         ...
         if ($contribute->rowCount() > 0) {
            while ($fetch_contribute= $contribute->fetch(PDO::FETCH_ASSOC)) {
               ...
               $select_admin->execute([$fetch_contribute['centre_id']]);
